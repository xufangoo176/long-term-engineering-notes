...
### 优化方式1️⃣：
新建一个字段，存储原数据的倒叙，然后给这个新字段添加索引，并使用 like 模糊查询，把 '%' 顺利的放到右边进行查询，此时成功的避免了左边 '%' 导致的索引失效。
<br/>
e.g.
1. 系统中已有 column_A 存储 '123abc' 的数据，当使用 like '%abc' 的时候，索引失效了。
...
tips： 虚拟列可以指定为 VIRTUAL或STORED，VIRTUAL 不会将虚拟列存储到磁盘中，在使用时 MySQL 会现场计算虚拟列的值，STORED 会存储到磁盘中，相当于我们手动创建的冗余列。所以：如果你的磁盘足够大，可以使用 STORED 方式，这样在查询时速度会更快一些。
